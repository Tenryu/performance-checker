 ...
 * PerformanceChecker
 */
class PerformanceCheckerModule extends eventemitter3_1.EventEmitter {
    constructor(option) {
        super();
        this.intervalId = -1;
    ...
    start() {
        ...
        this.intervalId = window.setInterval(() => {
            ...
            const average = {
                ...
                memory: list.length === 1 ? currentData.memory : PerformanceCheckerModule.calcAverage(this.data.average.memory, currentData.memory),
            };
            const max = {
                timestamp: 0,
                fps: list.length === 1 ? currentData.fps : PerformanceCheckerModule.calcMax(this.data.average.fps, currentData.fps),
                memory: list.length === 1 ? currentData.memory : PerformanceCheckerModule.calcMax(this.data.average.memory, currentData.memory),
            };
            const min = {
                timestamp: 0,
                fps: list.length === 1 ? currentData.fps : PerformanceCheckerModule.calcMin(this.data.average.fps, currentData.fps),
                memory: list.length === 1 ? currentData.memory : PerformanceCheckerModule.calcMin(this.data.average.memory, currentData.memory),
            };
            this.data.list = list;
